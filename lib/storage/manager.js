...
StorageManager.prototype._initShardReaper = function () {
  ...
  setTimeout(this.clean.bind(this, this._initShardReaper.bind(this)), constants.CLEAN_INTERVAL);
};

StorageManager.prototype.multipleDelete = function (hashList, callback) {
  async.eachSeries(hashList, (hash, nextHash) => {
    self._logger.info('Cleaner: Removing shard %s ...', hash);

    this._storage.del(hash, () => nextHash());
  }, callback);
}

/**
 * Get a list of actual shards stored in the farmer.
 * @param {*} options 
 * @param {*} callback 
 */
StorageManager.prototype.readShards = function (options, callback) {
  const self = this;
  const db = this._storage.createReadStream(options);

  let arr = [];
  let lastItem = null;

  db.on('data', function (item) {
    db.pause();

    self._storage.existDataShard(item.fskey, (err, exists) => {
      if (err) {
        self._logger.warn('Error on existsDataShard. Reason: %s', err.message);
      }
  ...
  db.on('end', () => callback(null, arr, lastItem));
};
