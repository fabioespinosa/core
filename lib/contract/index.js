...
Contract.prototype.signExternal = function(secret) {
  ...
  var signobj = secp256k1.ecdsaSign(hash,
    Buffer.from(keypair.getPrivateKeyPadded(), 'hex'));
...
Contract.prototype.verifyExternal = function(signature, pubkeyhash) {
  ...
  try {
    ...
    var pubKey = secp256k1.ecdsaRecover(sig, recovery, magic, true);
  } catch (err) {
    return false;
  }
  return secp256k1.ecdsaVerify(sig, magic, pubKey) &&
    bitcore.crypto.Hash.sha256ripemd160(Buffer.from(pubKey)).toString('hex') === pubkeyhash;
};
