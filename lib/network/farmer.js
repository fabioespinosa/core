...
FarmerInterface.CLEANER_INITIALIZED = false;
...
FarmerInterface.prototype.initCleaner = function (callback) {
  if (FarmerInterface.CLEANER_INITIALIZED) {
    return callback(Error('Cleaner already initialized'));
  }

  self._logger.info('Cleaning shards started');

  FarmerInterface.CLEANER_INITIALIZED = true;
  const streamOptions = { limit: 50 };
  this.performClean(streamOptions, (err) => {
    FarmerInterface.CLEANER_INITIALIZED = false;
    callback(err);
  });
}
...
const differenceBy = (array1, array2, key) => {
  return array1.filter(a => !array2.some(b => key ? (b[key] === a[key]) : (b === a)))
}

FarmerInterface.prototype.performClean = function (streamOptions, callback) {
  const self = this;

  self.storageManager.readShards(streamOptions, (err, localShards, lastShard) => {
    if (err) {
      return callback(err);
    }

    if (lastShard === streamOptions.gt) {
      // Last shard checked
      return callback();
    }

    streamOptions.gt = lastShard;

    if (localShards.length === 0) {
      // None of the contracts checked has a shard, so no action should be performed, continue
      return this.performClean(streamOptions, callback);
    }

    this.getShardsFromBridge(localShards, (err, bridgeShards) => {
      if (err) {
        return callback(err);
      }

      const diff = differenceBy(localShards, bridgeShards);

      return self.storageManager.multipleDelete(diff, (err) => {
        if (err) {
          return callback(err);
        }

        this.performClean(streamOptions, callback);
      });
    });
  });
}
...
FarmerInterface.prototype.getShardsFromBridge = function (shards, callback) {
  ...
  this._getShardsFromBridge(bridgeIter.next().value, shards, callback);
}
