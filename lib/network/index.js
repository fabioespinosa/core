...
const TriggerManager = require('../sips').SIP0003.TriggerManager;
const OfferManager = require('../contract/offer-manager');
 ...
 * @param {String} options.bridges - An array of bridges to connect
...
Network.STORJ_BRIDGE_KEY = 'xpub69vZdaeqerAfCThNZu6Zifz5K8XvFiPxG6UHZck5NC9ezmo48D8WmwhKYWjyxuwQVA3otsBZ8NFqfKvSGfLdY1qvTC75ZYUeYhJQdgdCgWJ';

Network.DEFAULTS = {
  bridges: [
    {
      url: process.env.STORJ_BRIDGE || 'https://api.internxt.com',
      extendedKey: process.env.STORJ_BRIDGE_KEY || Network.STORJ_BRIDGE_KEY
    }
  ],
  seedList: [],
...
Network.prototype._getContactList = function(callback) {

  if (!this._options.bridges ||
      !this._options.bridges[0] ||
      !this._options.bridges[0].url) {
    return callback(new Error('No bridges configured'));
  }
  const baseURI = this._options.bridges[0].url;

  this._logger.info('resolving contacts from %s', baseURI);
...
Network.prototype._enterOverlay = function(onConnected) {
  ...
  this._getContactList(function(err, seeds) {
    if (err) {
      return onConnected(
...
Network.prototype._verifyMessage = function(message, contact, callback) {
  ...
  this._validateContact(contact, function(err) {
    ...
    var addr = bitcore.Address.fromPublicKeyHash(
      Buffer.from(contact.nodeID, 'hex')
    );
...
Network.prototype._verifySignature = function(options, callback) {
  ...
  if (!pubKey) {
    try {
      pubKey = secp256k1.ecdsaRecover(sig, recovery, magic, true);
  ...
  if (!secp256k1.ecdsaVerify(sig, magic, pubKey)) {
...
Network.prototype._createSignatureObject = function(signature) {
  ...
  try {
    compactSig = Buffer.from(signature, 'base64');
...
Network.prototype._listenForTunnelers = function() {
  ...
  var prefix = Buffer.from([constants.OPCODE_TUNNELER_PREFIX], 'hex');
  var available = Buffer.from([constants.OPCODE_DEG_LOW], 'hex');
  var unavailable = Buffer.from([constants.OPCODE_DEG_NULL], 'hex');
...
Network.prototype._setupTunnelClient = function(callback) {
  ...
  if (!neighbors.length) {
    self._getContactList(function(err, result) {
      if (err) {
